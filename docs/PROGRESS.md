...
- **Some packages are one release behind the newest.** pnpm 11 won't install a version less than a day old (its minimum release age), so where the newest was that young, the one before it is pinned. Currently: expo 57.0.21 (newest 57.0.22), expo-router 57.0.20 (57.0.21), expo-linking 57.0.9 (57.0.10), expo-constants 57.0.17 (57.0.18) and use-intl 4.14.3 (4.14.4). Bump them once they qualify. (vite was held at 8.2.2 the same way, and is now 8.3.0.)
...
- The first Vitest run in a package after a fresh install can be very slow on this machine: cold imports took 13.5 seconds for `ui-web`, and one first run hit Vitest's 60-second worker start-up timeout before any test ran. A warm rerun takes seconds. If a worker times out on a first run, run the tests again before investigating further.
- The local Docker Compose stack was stopped from outside this session at 13:02 UTC on 2026-09-11 (a clean stop, not a crash). It was restarted with `pnpm infra:up` at about 16:20 UTC for the `pnpm dev` check, and is running; stop it with `pnpm infra:down`.
- Turbo 2 passes only declared environment variables to tasks. The M0 `pnpm build` and `pnpm dev` checks ran with `--env-mode=loose`, so the telemetry opt-outs (`EXPO_NO_TELEMETRY`, `NEXT_TELEMETRY_DISABLED`) reached Expo and Next.js. Step 9 declares the variables properly.
...
1. Step 9: CI. Install, lint, typecheck, unit tests with coverage thresholds, integration tests (Testcontainers), build, the OpenAPI drift check (`pnpm api:generate`, then `git diff --exit-code packages/api-client`), dependency-cruiser, the migration immutability check and `pnpm audit --audit-level high`, plus a smoke job. Actions are pinned to commit SHAs (looked up on 2026-09-11: actions/checkout v7.0.1 `3d3c42e`, actions/setup-node v7.0.0 `8207627`, pnpm/action-setup v6.1.0 `ea17c68`). Also declare Turbo's environment variables, and prove that a change to the shared ESLint preset re-runs every package's lint: `@voltdrop/config` has no build task, so Turbo may not count it as an input to other packages' cached lint results.
2. Step 10: the Claude Code setup (`CLAUDE.md`, rules, skills, agents, hooks and permissions).
