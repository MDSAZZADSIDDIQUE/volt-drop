...
1. Finish step 8: merchant-portal and admin (Vite), customer-web (Next.js), `ui-native` and merchant-app (Expo), then `pnpm dev` and `pnpm build` across everything.
2. Step 9: CI, including the OpenAPI drift check (`pnpm api:generate`, then `git diff --exit-code packages/api-client`). Also prove that a change to the shared ESLint preset re-runs every package's lint: `@voltdrop/config` has no build task, so Turbo may not count it as an input to other packages' cached lint results.
