...
- **Edit denies as a second layer.** The protected patterns that have no exceptions also have `Edit` deny rules: `*.pem`, `*.key`, `*.tfstate`, `*.tfstate.*`, `.terraform/**`, `/packages/api-client/src/generated/**` and `/apps/api/drizzle/meta/**`. Unlike the hook, deny rules also cover shell redirections and the file commands Claude Code recognises. `.env.*` can't have one, because `.env.example` must stay editable.
- **Why two path styles.** A bare pattern such as `*.pem` matches at any depth below the working directory, which is what a file-name rule wants. A single leading slash, as in `/packages/api-client/src/generated/**`, anchors at the settings file's own project root, which is what a rule about one folder wants; two leading slashes would mean the filesystem root. Both styles are deliberate.
- **Generated folders.** The hook and the deny rules cover `packages/api-client/src/generated/` and `apps/api/drizzle/meta/`. New migration SQL stays editable: only drizzle-kit's journal and snapshots are off limits.
